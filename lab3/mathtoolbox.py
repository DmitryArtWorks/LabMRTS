...
def apply_ideal_filter(t_d: int | float, 
    ...
    if isinstance(f_cut_hz, float or int): # Если одно значение частоты среза, то это будет или ФНЧ, или ФВЧ.
        ...
        f_cut_l = np.argmax(f_axis > f_cut_hz[0])   # Принцип тот же, как и в ветке с одной частотой среза. 
        f_cut_h = np.argmax(f_axis > f_cut_hz[1])   # Однако поскольку у данных фильтров две частоты среза, 
                                                    # необходимо найти индексы, соответствующие этим двум
                                                    # частотам. 
        if (filter_type == 'BP'):
            filter_f_char[:f_cut_l] = 0 # Если необходимо сформировать АЧХ ПФ, то все отсчеты, соответствующие
            filter_f_char[f_cut_h:] = 0 # частотам ниже f_cut_l и выше f_cut_h должны равняться нулю.
        elif (filter_type == 'BS'): 
            filter_f_char[f_cut_l:f_cut_h] = 0  # Если необходимо сформировать АЧХ ПФ, то все отсчеты, соответствующие
                                                # интервалу частот f_cut_l...f_cut_h должны равняться нулю.
        else:   
            raise ValueError('Ошибка вввода: Тип фильтра')  # Вызов исключения, если был передан неизвестный
                                                            # аргумент.
    else:
